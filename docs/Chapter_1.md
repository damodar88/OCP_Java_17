...
- java: Ejecuta el programa no se pone la extencion .class de bitecode o dara error. La JVM busca busca el archivo
  correspondiente a la clase principal.

```
java hola
```

- jar: El .jar se hace con el comando jar o con una herramienta Maven/Gradle. Es para empaquetar varios archivos en un
  archivo clases .jar
...
Campos pueden ser de la clase o del metodo, son las variables que tomaran los datos para ser procesados. Los de la clase 
son variables que pertenecen a la clase pero pueden ser usadas por los metodos de la clase, son globales de la clase. Los
de los metodos solo viven en los metodos y no pueden ser usadas por otros metodos. Igual estan las que viven en las
estructuras de control de flujo son de los metodos peror viven dentro de esas estructuras de control.

##### Comentarios

Los comentarios los ignora el compilador y no afectan al codigo. Es para documentar el codigo. Se suele usar el comentario
de una linea ``//`` y para varias lineas ``/* */`` y el javadoc ``/** */`` tambien esta el """ """, estos son comentarios
de bloque.

#### Clases y archivos fuente

Las clases son los archivos fuente se usan en mayuscula y tienen el mismo nombre del archivo y tienen la extencion .java.
Usa el modificador de acceso public para que sea accesible desde cualquier parte del programa y puede tener mas de una
clase en un mismo archivo pero solo uno public con el nombre de la clase por archivo. En java la clase solo puede tener 
un unico nombre de dentro del mismo package. Lo que si puedes tener es que clases anidadas compartan nombres de clases 
de nivel superior.

```
public class Config { // Clase Superior 1
   
}

class Producto { // Clase de Nivel Superior 2 (package-private)
    class Config { 
        // Esta bien: El nombre completo de esta clase anidada es Producto.Config
    }
}
```
...
nombre(por ejemplo, java.util.Date y java.sql.Date), debemos usar el nombre completo de la clase(su ruta completa o nombre
totalmente calificado) para evitar errores
de compilacion.
